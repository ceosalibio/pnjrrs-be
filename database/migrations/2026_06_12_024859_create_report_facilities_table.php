...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('report_facilities', function (Blueprint $table) {
            ...
            // $table->foreign('category_id', 'fk_report_facilities_category_id')
            //     ->references('id')->on('pn_categories')->onDelete('cascade');
            // $table->foreign('unit_id', 'fk_report_facilities_unit_id')
            //     ->references('id')->on('pn_units')->onDelete('cascade');
            // $table->foreign('sub_unit_id', 'fk_report_facilities_sub_unit_id')
            //     ->references('id')->on('pn_sub_units')->onDelete('set null');
            // $table->foreign('office_id', 'fk_report_facilities_office_id')
            //     ->references('id')->on('pn_offices')->onDelete('set null');
            // $table->foreign('sub_office_id', 'fk_report_facilities_sub_office_id')
            //     ->references('id')->on('pn_sub_offices')->onDelete('set null');
            // $table->foreign('created_by', 'fk_report_facilities_created_by')
            //     ->references('id')->on('users')->onDelete('set null');
            // $table->foreign('updated_by', 'fk_report_facilities_updated_by')
            //     ->references('id')->on('users')->onDelete('set null');
        });
    }
