...
class ReportEquipmentRepository
{
    public function all(): Collection
    {
        return ReportEquipment::all();
    }

    public function findById(int $id): ?ReportEquipment
    {
        return ReportEquipment::find($id);
    }

    public function create(array $data): ReportEquipment
    {
        return ReportEquipment::create($data);
    }

    public function update(int $id, array $data): ?ReportEquipment
    {
    ...
    {
        return ReportEquipment::paginate($perPage);
    }

    public function filterByUnitId(int $unitId, int $perPage = 15)
    {
        return ReportEquipment::where('unit_id', $unitId)->paginate($perPage);
    }

    public function filterBySubUnitId(int $subUnitId, int $perPage = 15)
    {
        return ReportEquipment::where('sub_unit_id', $subUnitId)->paginate($perPage);
    }

    public function filterByOfficeId(int $officeId, int $perPage = 15)
    {
        return ReportEquipment::where('office_id', $officeId)->paginate($perPage);
    }

    public function filterBySubOfficeId(int $subOfficeId, int $perPage = 15)
    {
        return ReportEquipment::where('sub_office_id', $subOfficeId)->paginate($perPage);
    }

    public function filterByMultiple(array $filters, int $perPage = null, $first = false)
    {
        $query = ReportEquipment::query();
